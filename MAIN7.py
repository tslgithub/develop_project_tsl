...
def find_mid_line(d, img):
    mat=img
    w,h =mat.shape[::-1]
    d.p2 = mat = mat[:,int(0.45*w):int(0.55*w)]
...
def find_circles(d,img,dir, isleft):
    ...
    for circle in circles[0,:]:
        ...
        roi_adaptive_weak[slice(int(y - r-20), int(y + r+20)), slice(0, width)]=0
...
def find_content2(d,img,dir):  #寻找二级目录,有二级目录，则调用，否则不调用
    ...
    d.p2=img = img[slice(0,h_origin-200),:]
    ...
    ContentPath2,Hight2,PagePath2,ClassPath2=[],[],[],[]
